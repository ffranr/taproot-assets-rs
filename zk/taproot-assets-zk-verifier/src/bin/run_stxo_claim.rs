...
/// Runs the STXO claim verifier against the first proof in the local proof file.
...
fn run(desired_threads: usize, available_threads: usize) -> anyhow::Result<()> {
    ...
    let anchor_input = zk_core::verify::tx::AnchorClaimInput {
    ...
    let anchor_output = zk_core::verify::tx::verify_anchor_claim(&anchor_input)?;

    let input = zk_core::verify::proof::StxoClaimInput {
    ...
    let output = zk_core::verify::proof::verify_stxo_claim(&input)
        .map_err(|err| anyhow::anyhow!("stxo claim verification failed: {}", err))?;
    println!("stxo claim verified? true");
    println!("stxo claim host output: {:?}", output);

    let prover = default_prover();
    let elf = std::fs::read(Path::new(
        "target/riscv32im-risc0-zkvm-elf/docker/stxo-claim-prover.bin",
    ))?;
    let env = ExecutorEnv::builder().write(&input)?.build()?;
    let receipt = prover.prove(env, &elf)?.receipt;
    let output: zk_core::verify::proof::StxoClaimOutput = receipt.journal.decode()?;
    println!("stxo claim receipt ok? true");
